 ...
 *  1 ------ 2
 */
UIImage *QPHourGlassThumbImage(CGSize size, CGFloat cWidth) {
 ...
 * +-----+
 */
UIImage *QPArrowLoopThumbImage(CGSize size, CGSize loopSize) {
...
@implementation QPBrightnessSlider
...
- (void)initRoutine {
  ...
  [self addTarget:self
                action:@selector(myValueChanged:)
...
- (void)drawRect:(CGRect)rect {
  ...
  CGContextDrawLinearGradient(ctx, myGradient, CGPointZero,
                              CGPointMake(rect.size.width, 0), 0);
  CGGradientRelease(myGradient);
  CGColorSpaceRelease(space);
}

- (void)setColorPicker:(QPColorTemperaturePickerView *)cp {
  _colorPicker = cp;
  if (!_colorPicker) {
    return;
  }
