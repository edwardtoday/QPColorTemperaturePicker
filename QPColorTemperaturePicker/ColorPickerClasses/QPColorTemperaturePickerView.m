...
- (void)initRoutine {
  ...
  self.selectionLayer.frame =
      CGRectMake(0.0, 0.0, kSelectionViewSize, kSelectionViewSize);
  ...
  self.selectionColorLayer.frame =
      CGRectMake(0.0, 0.0, kSelectionViewSize, kSelectionViewSize);
...
- (void)resizeOrRescale {
  if (!self.window || self.frame.size.width == 0 ||
  ...
  self.opacityLayer.backgroundColor = [[UIColor
      colorWithPatternImage:
          QPOpacityBackgroundImage(
              20, self.scale, [UIColor colorWithWhite:0.5 alpha:1.0])] CGColor];
...
- (void)setFrame:(CGRect)frame {
  NSAssert(frame.size.width == frame.size.height,
           @"QPColorTemperaturePickerView must be square.");
...
- (void)genBitmap {
  if (!_colorPickerViewFlags.bitmapNeedsUpdate)
    return;

  self.rep = [self.class bitmapForDiameter:self.gradientLayer.bounds.size.width
                                     scale:self.scale
                                   padding:self.paddingDistance
                               shouldCache:YES];
  _colorPickerViewFlags.bitmapNeedsUpdate = NO;
  self.gradientLayer.contents =
      (id)[QPUIImageWithScale(self.rep.image, self.scale) CGImage];
}
...
- (void)generateBezierPaths {
  ...
  CGRect activeAreaFrame =
      CGRectInset(self.bounds, self.paddingDistance, self.paddingDistance);
  if (self.cropToCircle) {
    self.contentsLayer.cornerRadius = self.paletteDiameter / 2.0;
    self.activeAreaShape =
        [UIBezierPath bezierPathWithOvalInRect:activeAreaFrame];
  } else {
...
- (CGPoint)selection {
  return [state selectionLocationWithSize:self.paletteDiameter
                                  padding:self.paddingDistance];
}
...
- (void)setCropToCircle:(BOOL)circle {
  ...
  if (circle) {
    ...
    CGPoint point = [self
        validPointForTouch:[state
                               selectionLocationWithSize:self.paletteDiameter
                                                 padding:self.paddingDistance]];
...
- (void)handleStateChangedDisableActions:(BOOL)disable {
  ...
  CGPoint selectionLocation =
      [state selectionLocationWithSize:self.paletteDiameter
                               padding:self.paddingDistance];
  ...
  loupeFrame.origin =
      CGPointMake(round(loupeFrame.origin.x), round(loupeFrame.origin.y));
  ...
  // notify delegate
  if ([self.delegate
          respondsToSelector:@selector(colorPickerDidChangeSelection:)]) {
...
- (CGPoint)validPointForTouch:(CGPoint)touchPoint {
  ...
  if (self.cropToCircle) {
    ...
    // TouchCircle is the circle which pass by the point 'touchPoint', of radius
    // 'r'
    ...
    if (touchPoint.y > CGRectGetMidX(self.bounds))
      alpha = (2 * M_PI) - alpha;

    // 'actual radius' is the distance between the center and the border of the
    // gradient
    ...
    // we offset the center of the circle, to get the coordinate from the right
    // top left origin
    ...
    if (point.x < self.paddingDistance)
      point.x = self.paddingDistance;
    if (point.x > self.paletteDiameter - self.paddingDistance) {
      point.x = self.paletteDiameter - self.paddingDistance;
    }
    if (point.y < self.paddingDistance)
      point.y = self.paddingDistance;
...
- (QPColorTemperaturePickerState *)stateForPoint:(CGPoint)point {
  QPColorTemperaturePickerState *newState =
      [QPColorTemperaturePickerState stateForPoint:point
                                              size:self.paletteDiameter
                                           padding:self.paddingDistance];
  newState = [[newState stateBySettingAlpha:self.opacity]
      stateBySettingBrightness:self.brightness];
  return newState;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if ([self.delegate
          respondsToSelector:@selector(colorPicker:touchesBegan:withEvent:)]) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if ([self.delegate
          respondsToSelector:@selector(colorPicker:touchesEnded:withEvent:)]) {
...
+ (void)initialize {
  ...
  generateQueue.maxConcurrentOperationCount =
      NSOperationQueueDefaultMaxConcurrentOperationCount;
  backgroundQueue = dispatch_queue_create(
      "com.github.rsully.QPColorTemperaturePicker.background",
...
+ (void)prepareForDiameter:(CGFloat)diameter scale:(CGFloat)scale {
  [self prepareForDiameter:diameter
                     scale:scale
                   padding:kSelectionViewSize / 2.0];
}
...
+ (void)prepareForDiameter:(CGFloat)diameter
  ...
  void (*function)(dispatch_queue_t, dispatch_block_t) =
      bg ? dispatch_async : dispatch_sync;
  function(backgroundQueue, ^{
      [self bitmapForDiameter:diameter
                        scale:scale
                      padding:padding
                  shouldCache:YES];
  });
...
+ (QPImageBitmapRep *)bitmapForDiameter:(CGFloat)diameter
                                  scale:(CGFloat)scale
                                padding:(CGFloat)paddingDistance
                            shouldCache:(BOOL)cache {
  QPGenerateOperation *repOp = nil;
  ...
  NSString *dictionaryCacheKey =
      [NSString stringWithFormat:@"%.1f-%.1f", diameter, paddingDistance];
  // Check cache
  ...
  repOp = [[QPGenerateOperation alloc] initWithDiameter:diameter
                                             andPadding:paddingDistance];
